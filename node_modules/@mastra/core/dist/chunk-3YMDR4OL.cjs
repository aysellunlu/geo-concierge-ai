  ...
  try {
    ...
    if (globalDtsExists) {
      ...
      if (/readonly\s+\d/.test(globalDtsContent)) {
        console.warn(
          `[GatewayRegistry] Detected invalid provider-types in global cache at ${GLOBAL_PROVIDER_TYPES_DTS()}. Deleting corrupted file.`
        );
        try {
          fs__default.default.unlinkSync(GLOBAL_PROVIDER_TYPES_DTS());
        } catch {
        }
      } else {
...
function getPackageRoot() {
  try {
    const require2 = module$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('chunk-3YMDR4OL.cjs', document.baseURI).href)) || "file://");
...
var GatewayRegistry = class _GatewayRegistry {
  ...
  async syncGateways(forceRefresh = false, writeToSrc = false) {
    ...
    try {
      const { ModelsDevGateway } = await import('./models-dev-BW2GAM3K.cjs');
      const { NetlifyGateway } = await import('./netlify-V5F7JEJH.cjs');
      const { fetchProvidersFromGateways, writeRegistryFiles } = await import('./registry-generator-WLVXJJFY.cjs');
...
//# sourceMappingURL=chunk-3YMDR4OL.cjs.map
//# sourceMappingURL=chunk-3YMDR4OL.cjs.map