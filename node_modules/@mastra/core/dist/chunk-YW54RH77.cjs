...
var AIV5Adapter = class {
  ...
  static fromUIMessage(uiMsg) {
    ...
    const toolInvocationParts = parts.filter((p) => chunkZIEQA3YP_cjs.isToolUIPart(p));
    ...
    const v2Parts = parts.map((p) => {
      if (chunkZIEQA3YP_cjs.isToolUIPart(p)) {
        const toolName = getToolName(p);
...
var CacheKeyGenerator = class _CacheKeyGenerator {
  ...
  static fromAIV5Parts(parts) {
    ...
    for (const part of parts) {
      ...
      if (chunkZIEQA3YP_cjs.isToolUIPart(part) || part.type === "dynamic-tool") {
        key += part.toolCallId;
        key += part.state;
      }
      if (part.type === "reasoning") {
...
function sanitizeV5UIMessages(messages, filterIncompleteToolCalls = false) {
  const msgs = messages.map((m) => {
    ...
    const safeParts = m.parts.filter((p) => {
      ...
      if (!chunkZIEQA3YP_cjs.isToolUIPart(p)) return true;
    ...
    const sanitized = {
      ...
      parts: safeParts.map((part) => {
        if (chunkZIEQA3YP_cjs.isToolUIPart(part) && part.state === "output-available") {
          return {
            ...part,
...
function addStartStepPartsForAIV5(messages) {
  for (const message of messages) {
    ...
    for (const [index, part] of message.parts.entries()) {
      if (!chunkZIEQA3YP_cjs.isToolUIPart(part)) continue;
      const nextPart = message.parts.at(index + 1);
      if (nextPart && nextPart.type !== `step-start` && !chunkZIEQA3YP_cjs.isToolUIPart(nextPart)) {
...
function aiV4UIMessagesToAIV4CoreMessages(messages) {
  return chunkCC3OOESY_cjs.convertToCoreMessages(sanitizeAIV4UIMessages(messages));
}
function aiV5UIMessagesToAIV5ModelMessages(messages, dbMessages, filterIncompleteToolCalls = false) {
  ...
  const result = chunkZIEQA3YP_cjs.convertToModelMessages(preprocessed);
...
var StepContentExtractor = class _StepContentExtractor {
  ...
  static convertPartsToContent(parts, stepId, stepContentFn) {
    ...
    const modelMessages = chunkZIEQA3YP_cjs.convertToModelMessages(sanitizeV5UIMessages(stepUiMessages));
...
exports.AIV4Adapter = AIV4Adapter;
exports.AIV5Adapter = AIV5Adapter;
...
exports.MessageMerger = MessageMerger;
exports.MessageStateManager = MessageStateManager;
exports.TypeDetector = TypeDetector;
exports.aiV4CoreMessageToV1PromptMessage = aiV4CoreMessageToV1PromptMessage;
exports.aiV5ModelMessageToV2PromptMessage = aiV5ModelMessageToV2PromptMessage;
exports.convertMessages = convertMessages;
exports.coreContentToString = coreContentToString;
exports.ensureAnthropicCompatibleMessages = ensureAnthropicCompatibleMessages;
exports.ensureGeminiCompatibleMessages = ensureGeminiCompatibleMessages;
exports.findToolCallArgs = findToolCallArgs;
exports.getOpenAIReasoningItemId = getOpenAIReasoningItemId;
exports.hasOpenAIReasoningItemId = hasOpenAIReasoningItemId;
exports.messagesAreEqual = messagesAreEqual;
//# sourceMappingURL=chunk-YW54RH77.cjs.map
//# sourceMappingURL=chunk-YW54RH77.cjs.map