...
var CoreToolBuilder = class extends chunkRO47SMI7_cjs.MastraBase {
  originalTool;
  options;
  logType;
  constructor(input) {
    ...
    if (!chunkEAZ6YDCQ_cjs.isVercelTool(this.originalTool) && (input.autoResumeSuspendedTools || this.originalTool.id?.startsWith("agent-"))) {
      ...
      if (chunkBFIOQFGF_cjs.isZodObject(schema)) {
        this.originalTool.inputSchema = schema.extend({
          suspendedToolRunId: zod.z.string().describe("The runId of the suspended tool").nullable().optional().default(""),
          resumeData: zod.z.any().describe("The resumeData object created from the resumeSchema of suspended tool").optional()
        });
      }
    }
  }
  ...
  getParameters = () => {
    if (chunkEAZ6YDCQ_cjs.isVercelTool(this.originalTool)) {
  ...
  createExecute(tool, options, logType, processedSchema) {
    ...
    const execFunction = async (args, execOptions) => {
      ...
      try {
        ...
        if (chunkEAZ6YDCQ_cjs.isVercelTool(tool)) {
          result = await chunkKRAGJ433_cjs.executeWithContext({
            span: toolSpan,
            fn: async () => tool?.execute?.(args, execOptions)
          });
        } else {
          const wrappedMastra = options.mastra ? chunkKRAGJ433_cjs.wrapMastra(options.mastra, { currentSpan: toolSpan }) : options.mastra;
          ...
          const baseContext = {
            ...
            runId: options.runId,
            requestContext: execOptions.requestContext ?? options.requestContext ?? new chunkVDKWYUGC_cjs.RequestContext(),
            ...
            suspend: (args2, suspendOptions) => {
              ...
              const newSuspendOptions = {
                ...suspendOptions ?? {},
                resumeSchema: suspendOptions?.resumeSchema ?? (resumeSchema ? JSON.stringify(zodToJson.zodToJsonSchema(resumeSchema)) : void 0)
              };
          ...
          if (resumeData) {
            const resumeValidation = chunkEAZ6YDCQ_cjs.validateToolInput(resumeSchema, resumeData, options.name);
          ...
          result = await chunkKRAGJ433_cjs.executeWithContext({ span: toolSpan, fn: async () => tool?.execute?.(args, toolContext) });
        }
        if (suspendData) {
          const suspendSchema = this.getSuspendSchema();
          const suspendValidation = chunkEAZ6YDCQ_cjs.validateToolSuspendData(suspendSchema, suspendData, options.name);
        ...
        if (chunkEAZ6YDCQ_cjs.isVercelTool(tool)) {
          ...
          const outputValidation = chunkEAZ6YDCQ_cjs.validateToolOutput(outputSchema, result, options.name, false);
    ...
    return async (args, execOptions) => {
      ...
      try {
        ...
        const { data, error: error2 } = chunkEAZ6YDCQ_cjs.validateToolInput(parameters, args, options.name);
        if (error2) {
  ...
  build() {
    ...
    let needsApprovalFn;
    if (chunkEAZ6YDCQ_cjs.isVercelTool(this.originalTool) && "needsApproval" in this.originalTool) {
    ...
    return {
      ...
      parameters: processedSchema ?? zod.z.object({}),
...
function ensureToolProperties(tools) {
  const toolsWithProperties = Object.keys(tools).reduce((acc, key) => {
    ...
    if (tool) {
      if (typeof tool === "function" && !(tool instanceof chunkEAZ6YDCQ_cjs.Tool) && !chunkEAZ6YDCQ_cjs.isVercelTool(tool)) {
...
//# sourceMappingURL=chunk-QDH6MVJ7.cjs.map
//# sourceMappingURL=chunk-QDH6MVJ7.cjs.map