...
var chunkCQ4Z6ZAV_cjs = require('./chunk-CQ4Z6ZAV.cjs');
...
function createUndefinedPrimitiveError(type, value, key) {
  ...
  return new chunk4U7ZLI36_cjs.MastraError({
    id: errorId,
...
var Mastra = class {
  ...
  get datasets() {
    if (!this.#datasets) {
      this.#datasets = new chunk7WC7ALDW_cjs.DatasetsManager(this);
    }
   ...
   */
  constructor(config) {
    this.#serverCache = new chunk2VD5OGOT_cjs.InMemoryServerCache();
    ...
    if (config?.pubsub) {
      ...
      this.#pubsub = new chunk7S6LA43E_cjs.EventEmitterPubSub();
    }
    this.#events = {};
    ...
    const workflowEventProcessor = new chunkOQ5LZ6OV_cjs.WorkflowEventProcessor({ mastra: this });
    ...
    if (config?.logger === false) {
      ...
      if (config?.logger) {
        ...
        const levelOnEnv = process.env.NODE_ENV === "production" && process.env.MASTRA_DEV !== "true" ? chunk7XAECHYL_cjs.LogLevel.WARN : chunk7XAECHYL_cjs.LogLevel.INFO;
        logger = new chunk7XAECHYL_cjs.ConsoleLogger({ name: "Mastra", level: levelOnEnv });
      }
    }
    this.#logger = logger;
    ...
    if (storage) {
      storage = chunk7S6LA43E_cjs.augmentWithInit(storage);
    }
    if (config?.observability) {
      if (typeof config.observability.getDefaultInstance === "function") {
        ...
        this.#observability = new chunkKRAGJ433_cjs.NoOpObservability();
      }
    } else {
      this.#observability = new chunkKRAGJ433_cjs.NoOpObservability();
    }
    ...
    chunkMR7ZWBL6_cjs.registerHook("onScorerRun" /* ON_SCORER_RUN */, chunkCQ4Z6ZAV_cjs.createOnScorerHook(this));
  ...
  addAgent(agent, key, options) {
    ...
    let mastraAgent;
    if (chunkG5JVVFIG_cjs.isToolLoopAgentLike(agent)) {
      mastraAgent = chunkG5JVVFIG_cjs.toolLoopAgentToMastraAgent(agent, { fallbackName: key });
    } else {
      mastraAgent = agent;
    }
   ...
   */
  setStorage(storage) {
    this.#storage = chunk7S6LA43E_cjs.augmentWithInit(storage);
  }
  setLogger({ logger }) {
    this.#logger = logger;
    if (this.#agents) {
  ...
  #syncGatewayRegistry() {
    try {
      ...
      import('./provider-registry-4PH2JPIA.cjs').then(async ({ GatewayRegistry }) => {
...
//# sourceMappingURL=chunk-NUV3BLRH.cjs.map
//# sourceMappingURL=chunk-NUV3BLRH.cjs.map