...
var LocalFilesystem = class extends MastraFilesystem {
  ...
  resolvePath(inputPath) {
    let absolutePath;
    if (!this._contained && nodePath__namespace.isAbsolute(inputPath)) {
...
var LocalSkillSource = class {
  ...
  #resolvePath(skillPath) {
    if (nodePath__namespace.isAbsolute(skillPath)) {
      return skillPath;
    }
    return nodePath__namespace.resolve(this.#basePath, skillPath);
  }
  ...
  async readFile(skillPath) {
    ...
    const content = await fs2__namespace.readFile(resolved);
...
function isSeatbeltAvailable() {
  if (os__namespace.default.platform() !== "darwin") {
    return false;
  }
  return commandExists("sandbox-exec");
}
function isBwrapAvailable() {
  if (os__namespace.default.platform() !== "linux") {
    return false;
  }
  return commandExists("bwrap");
}
function detectIsolation() {
  const platform2 = os__namespace.default.platform();
  if (platform2 === "darwin") {
...
var LocalSandbox = class extends MastraSandbox {
  ...
  async destroy() {
    ...
    if (this._sandboxFolderPath) {
      try {
        await fs2__namespace.rmdir(this._sandboxFolderPath);
      } catch {
      }
  ...
  async getInfo() {
    return {
      ...
      resources: {
        memoryMB: Math.round(os__namespace.totalmem() / 1024 / 1024),
